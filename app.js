...
function createNumbersBlock() {
  ...
  numbersBlock.classList.add('number-block');
...
function checkValues() {
  ...
  userInputs.forEach((input, index) => {
    ...
    input.value === userInputs[index + 1].value
      ? (input.classList.add('correct'), input.classList.remove('error'))
      : (input.classList.add('error'), input.classList.remove('correct'));
  });
}
